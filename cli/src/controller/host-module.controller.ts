...
export class HostModuleController extends Controller {
  ...
  public async add(dependencyConfigPath?: string, dependencyModuleNames?: string[]): Promise<void> {
    ...
    dependencyModuleNames = await this.doAdd(dependencyConfigPath, dependencyModuleNames, cancelled, cfg);
  }

  private async doAdd(dependencyConfigPath: string, dependencyModuleNames: string[] | undefined, cancelled: boolean, cfg: GahHost) {
    ...
    if (!cfg.modules) { cfg.modules = new Array<ModuleReference>(); }
