...
export class GahHostDef extends GahModuleBase {
      ...
      for (const depModuleName of moduleDependency.names) {
        ...
        if (alreadyInitialized) {
          ...
          if (await this.fileSystemService.fileExists(moduleAbsoluteBasepath)) {
            const newModuleDef = new GahModuleDef(moduleAbsoluteBasepath, depModuleName, this.initializedModules, this.gahConfigs);
  ...
  private async cleanAndGenerateStylesScssFile() {
    ...
    await this.fileSystemService.saveFile(this.fileSystemService.join(this.basePath, this.srcBasePath, 'styles.scss'),
      ''
      + '/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n'
  ...
  private async referenceGlobalStyles() {
    const stylesScss = await this.fileSystemService.readFileLineByLine(this.fileSystemService.join(this.basePath, this.srcBasePath, 'styles.scss'));
    ...
    for (const dep of this.allRecursiveDependencies) {
      ...
      if (await dep.preCompiled()) {
        ...
        if (await this.fileSystemService.fileExists(absoluteStylesFilePathOfDep)) {
          ...
          const depStylesPathRelativeToSrcBase = await this.fileSystemService.ensureRelativePath(absoluteStylesFilePathOfDep, depAbsoluteSrcFolder, true);
          const dependencyPathRelativeFromSrcBase = await this.fileSystemService.ensureRelativePath(this.gahFolder.dependencyPath, this.srcBasePath, true);
    ...
    for (const dep of this.allRecursiveDependencies) {
      ...
      Object.keys(packageJson.scripts).forEach(scriptName => {
        if (scriptName.startsWith('gah-') && scriptName !== 'gah-preinstall' && scriptName !== 'gah-postinstall') {
          ...
          if (existingScript) {
            this.loggerService.warn(`The gah-script named "${simpleScriptName}" is declared multiple times. (${existingScript.moduleName} & ${dep.moduleName!})`);
          } else {
            allGahScripts.push(
              {
