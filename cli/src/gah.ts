...
export const gah = {
  ...
  install: (skipPackageInstall: boolean = false, configName?: string) =>
    DIContainer.resolve<InstallController>('installController')
      .install(skipPackageInstall, configName),
  /**
  ...
  run: (command: string, configurationName?: string) =>
    DIContainer.resolve<RunController>('runController')
      .exec(command.split(/\s+/), configurationName)
};
